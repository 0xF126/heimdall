var name_text = "Heimdall";
var last_upd = "25.04.21";
var version = "2.0.1";
var width = 720;
var height = 466;
var subtabs_width = width - (15 * 3);
const tab_margin = 150;
const tab_width = 50;
const header_radius = 0;
const element_margin_bottom = 7;
const checkbox_height = 12;
const checkbox_width = 12;
const box_margin_top = 18;
const box_margin_bottom = 18;
const box_margin_x = 8;
const box_text_margin_x = 29;
const slider_height = 8;
const slider_text_margin = 9;
const list_height = 25;
const line_color = [0, 0, 0, 255];
const bright_line_color = [0, 0, 0, 255];
const subtab_text_color = [255, 255, 255, 150];
const background_color = [0, 0, 0, 255];
const tab_active_color = [0, 0, 0, 255];
const text_color = [255, 255, 255, 255];
const text_color_heimdall = [0, 0, 0, 255];
const heimdall_tabcolor = [214, 65, 96, 255];
const element_color = [0, 0, 0, 255];
const element_border_color = [0, 0, 0, 255];
const element_border_color2 = [18, 18, 18, 255];
const element_active_color = [214, 65, 96, 255];
const hintbox_color = [0, 0, 0, 255];
const white_color = [255, 255, 255, 255];
const list_text_color = [255, 255, 255, 255];
    ...
    "a": {
        "General": {
            "General": [{
                ...
                "type": "checkbox",
                "name": "Safe point on limbs",
                "id": "safe_limbs",
				"hint": "Hits safe point mainly on limbs avoiding head, reversal aa."
            }, {
                "type": "checkbox",
                "name": "Jumpscout",
                "id": "jumpscout",
				...
				"hint": "Predicts use of autoscope when using scout/awp."
            }, {
                "type": "checkbox",
                "name": "Lethal safety",
                "id": "lethal_safety",
                "hint": "Forces baim or safe points when target is lethal"
            }, ],
            "Min-DMG bind": [{
                "type": "checkbox",
                "name": "Enabled",
                "id": "mindamage",
				"hint": "Creates bind for minimal damage override."
            }, {
                "type": "slider",
            ...
            "Other": [{
                "type": "checkbox",
                "name": "Force backshoot bind",
                "id": "force_backshoot",
				"hint": "Creates bind for backshoot."
            }, {
                "type": "checkbox",
                "name": "Ping spike bind",
                "id": "ping_spike",
				"hint": "Creates bind for fake ping."
            }, {
                "type": "checkbox",
                "name": "Custom zeus hitchance",
                "id": "zeus_hitchance_enabled",
				"hint": "Uses own hitchance for auto-zeus."
            }, {
                "type": "slider",
                "name": "Hitchance",
                "id": "zeus_hitchance",
            ...
            "Doubletap": [{
                ...
                "id": "doubletap_boost",
				"hint": "Faster doubletap when using auto awp."
            }, {
                "type": "checkbox",
                "name": "Matchmaking DT",
                "id": "mm_dt",
        ...
        "Anti-Aim": {
            ...
            "Auto invert": [{
                ...
                "id": "anti_brute",
                "hint": "Inverts your AA when enemy miss a shot"
            }],
            "Animations": [{
                "type": "checkbox",
                "name": "Slowmotion",
    ...
    "c": {
        "General": {
            "Miscellaneous": [{
                ...
                "name": "Clantag",
                "id": "clantag",
                "hint": "Enables Heimdall clan tag."
            }, {
                "type": "checkbox",
                "name": "Killsay",
                "id": "killsay",
                "hint": "Enables trashtalk on player kill."
            }, ],
            "Information": [{
                "type": "checkbox",
                "name": "Buy list",
                "id": "buy_list",
                "hint": "You will see all purchases on yours and enemy team."
            }, {
                "type": "checkbox",
                "name": "Vote revealer",
                "id": "vote_revealer",
                "hint": "You will see all votes around server(Yours, and Enemy team)"
            }, {
                "type": "checkbox",
                "name": "Watermark",
                "id": "watermark"
			...
			"Heimdall V2": [{
                ...
                "name": "Author ---------------------------->",
                "id": "",
				"hint": "panfr0st   DC: Guardian#6381"
            }, {
                "type": "checkbox",
                "name": "Special credits ---------------------------->",
                "id": "",
				"hint": "OTC Cord, HvH Essentials, Finkdao, Monolith, Kiera, Github"
            }, {
                "type": "checkbox",
                "name": "Beta v2 ---------------------------->",
                "id": "",
				"hint": "This is beta v2, so please use bug tracker at \nGithub.com/panfr0st/heimdall/issues/"
            }, ]
        }
    }
};
...
function autoscope() {
    ...
    if (checkbox_states["adaptive_noscope"]) {
        ...
        if (!Ragebot.GetTarget()) noscope_target = closestTarget();
        else noscope_target = Ragebot.GetTarget();
...
function bulletTracer() {
    ...
    var impact_x = Event.GetFloat("x"),
        impact_y = Event.GetFloat("y"),
        impact_z = Event.GetFloat("z");
    local = Entity.GetLocalPlayer();
...
function bulletTracer2() {
    ...
    if (wts_impact[2] === 1 && // some useless shit
        wts_eye_angles[2] === 1 && (Globals.Curtime() - render_time) < 4) Render.Line(wts_eye_angles[0], wts_eye_angles[1], wts_impact[0], wts_impact[1], UI.GetColor("Script items", "Bullet tracer"));
}
var enemies = Entity.GetEnemies();
var last_shot_time = []
...
function legBreaker() {
    ...
    if ((Globals.Tickcount() - legbreaker_old_tick_count) > legbreaker_speed && (!checkbox_states["alternative_fakelag"] || exploitsActive("all"))) {
        if (UI.GetValue("Misc", "GENERAL", "Movement", "Slide walk")) UI.SetValue("Misc", "GENERAL", "Movement", "Slide walk", 0);
        else UI.SetValue("Misc", "GENERAL", "Movement", "Slide walk", 1);
...
function hitShotsHandle() {
    ...
    for (shot in hitShots) {
        ...
        if (checkbox_states["damage_marker"]) {
            ...
            color[3] = alpha;
            var font = Render.AddFont("Inter", 13, 700);
...
function mmFD() {
    ...
    if (UI.IsHotkeyActive("Script items", "Matchmaking FD")) {
        ...
        if (height > fd_min_height[fd_type] && height < fd_max_height[fd_type]) {
            if (lock_camera) Convar.SetString('cl_lock_camera', '1');
...
function idealYaw() {
    ...
    for (var r = yaw - 90; r <= yaw + 90; r += 30) {
        ...
        const point = VectorOperate(origin, VectorNew([
            256 * Math.cos(rad),
            256 * Math.sin(rad),
            0
        ]), "+");
...
function buyList() {
    ...
    var add_height = 0;
    Render.GradientRect(buy_list_x, buy_list_y, buy_list_width / 2, 2, 1, [0, 142, 255, 255], [0, 142, 255, 255]);
    Render.GradientRect(buy_list_x + buy_list_width / 2, buy_list_y, buy_list_width / 2, 2, 1, [0, 142, 255, 255], [0, 142, 255, 255]);
    Render.FilledRect(buy_list_x, buy_list_y + 2, buy_list_width, list_height - 2, [0, 0, 0, 150]);
    list_font = Render.AddFont("Inter", 9, 900);
...
function voteRevealer() {
    ...
    Global.PrintColor([217, 217, 217, 255], "[Heimdall] \0");
...
function watermark() {
    ...
    Render.GradientRect(screen_size[0] - margin - watermark_width, watermark_y, watermark_width / 2, 2, 1, [0, 142, 255, 255], [0, 142, 255, 255]);
    Render.GradientRect(watermark_x + watermark_width / 2, watermark_y, watermark_width / 2, 2, 1, [0, 142, 255, 255], [0, 142, 255, 255]);
    Render.FilledRect(watermark_x, watermark_y + 2, watermark_width, list_height - 2, [0, 0, 0, 150]);
...
function eyeTracers() {
    ...
    for (var i = 0; i < playerList.length; i++) {
        if (!Entity.IsAlive(playerList[i]) || Entity.IsDormant(playerList[i])) continue;
        ...
        if (stop[2] != 1 || entityHead[2] != 1) continue;
...
function enemyLocation() {
    ...
    for (i = 0; i < enemies.length; i++) {
        if (Entity.IsAlive(enemies[i])) {
            ...
            var SendInfo = ("[Heimdall] Player " + Name + " with " + Health + " HP " + " is at " + Location + " with weapon " + Weapon);
...
function musicKit() {
    if (Cheat.FrameStage() != 1 || !checkbox_states["music_kit"]) return;
    Entity.SetProp(Entity.GetLocalPlayer(), "CCSPlayerResource", "m_nMusicID", GetValue("Music kit"));
}
var indicator_names = [
    ["DMG", ["Script items", "Min damage override"],
        [255, 133, 0, 255]
    ],
    ["DT", ["Rage", "GENERAL", "Exploits", "Doubletap"],
        [74, 207, 0, 255]
    ],
    ["HIDE", ["Rage", "GENERAL", "Exploits", "Hide shots"],
        [199, 34, 53, 255]
    ],
    ["ONSHOT", ["Script items", "Force backshoot"],
        [74, 207, 0, 255]
    ],
    ...
    ["BAIM", ["Rage", "GENERAL", "General", "Force body aim"],
        [199, 34, 53, 255]
    ],
    ["SAFE POINTS", ["Rage", "GENERAL", "General", "Force safe point"],
        [0, 222, 222, 255]
    ],
...
function indicators() {
    ...
    var active_binds = [];
    var binds_offset = 0;
    var font = Render.AddFont("Inter", 7, 400);
...
var heimdall_clantag = [" h", "he ", " hei", "heim ", " heimd", "heimda ", " heimdal", "heimdall ", " heimdall ™", "heimdall ™ ", " heimdall ™", "heimdall ™ ", " heimdall ™", "heimdall ™ ", " heimdall  ", "heimdall", " heimdal", "heimda ", " heimd", "heim ", " hei", "he ", " h", "h", " ", " ", ];
...
function clantag() {
    ...
    Local.SetClanTag(heimdall_clantag[(time) % heimdall_clantag.length]);
}
var script_items = {
    "lowdelta": [{
        "name": "Lowdelta",
    ...
    "lethal_safety": [{
        "name": "Lethal safety",
        "type": "multi",
        "elements": ["Force safe point", "Force baim"]
    }],
    "mm_fd": [{
        "name": "Matchmaking FD",
        "type": "hotkey"
    }, {
        "name": "Matchmaking FD type",
        "type": "dropdown",
        "elements": ["Legit (49-59)", "Semirage (46-56)", "Rage (47-55)", "Lower Rage (47-53)", "Semifull (47-59)", "Full (46-60)"]
    }, {
    ...
    "killsay": [{
        "name": "Killsay toxic mode",
        "type": "checkbox"
    }],
    "indicators": [{
        "name": "Desync strength indicator",
        "type": "checkbox"
    }, {
    ...
    "watermark": [{
        ...
        "type": "multi",
        "elements": ["Username", "Ping", "FPS", "Tickrate", name_text + " version", "Time"]
    }],
    "clantag": [{
        "name": "Clantag",
        "type": "dropdown",
...
UI.IsCursorInBox = function(x, y, width, height) {
    ...
    if (cursor[0] > x && cursor[0] < x + width && cursor[1] > y && cursor[1] < y + height) return true
    return false
}
        ...
        for (s = 0; s < subtab_names.length; s++) {
            ...
            for (b = 0; b < box_names.length; b++) {
                ...
                for (e = 0; e < box.length; e++) {
                    ...
                    switch (element["type"]) {
                        case "checkbox":
                            if (!loadedSettings) UI.AddCheckbox(element_name);
                            checkbox_states[element["id"]] = UI.GetValue("Script items", element_name);
                            break;
                        case "slider":
                            if (!loadedSettings) UI.AddSliderInt(element_name, element["min"], element["max"]);
                            if ("default" in element) {
                                if (!loadedSettings) UI.AddCheckbox(element_name + "_not_def");
                                if (!UI.GetValue("Script items", element_name + "_not_def")) UI.SetValue("Script items", element_name, element["default"]);
...
function drawMenu() {
    if (UI.IsMenuOpen()) {
        ...
        if (!block_set9) {
            Cheat.ExecuteCommand("fps_max 0");
            block_set9 = true;
        }
        return;
    }
    var logo_font = Render.AddFont("Valorant", 16, 400);
    icon_font = Render.AddFont("Heimdall Ui", 18, 500);
    text_icon_font = Render.AddFont("Inter", 10, 500);
    menu_font = Render.AddFont("Inter", 10, 400);
    heimdall_tab = Render.AddFont("Boldfinger", 12, 400);
    subtabs_font = Render.AddFont("Hakuna Sans", 9, 400);
    ...
    Render.Line(x, y + 8, x, y + height - 6, [19, 24, 37, 255]);
    Render.Line(x + width - 1, y + 8, x + width - 1, y + height - 6, [19, 24, 37, 255]);
    Render.Line(x, y + height - 5, x + 4, y + height - 1, [19, 24, 37, 255]);
    Render.Line(x + 4, y + height - 1, x + width - 5, y + height - 1, [19, 24, 37, 255]);
    Render.Line(x + width - 1, y + height - 5, x + width - 4, y + height - 2, [19, 24, 37, 255]);
    Render.Line(x + 1, y + 8, x + 1, y + height - 5, [19, 24, 37, 255]);
    Render.Line(x + width - 2, y + 8, x + width - 2, y + height - 6, [19, 24, 37, 255]);
    Render.Line(x + 1, y + height - 5, x + 4, y + height - 2, [19, 24, 37, 255]);
    Render.Line(x + 5, y + height - 2, x + width - 6, y + height - 2, [19, 24, 37, 255]);
    Render.Line(x + width - 2, y + height - 5, x + width - 6, y + height - 2, [19, 24, 37, 255]);
    Render.GradientRect(x + 1, y + 8, width - 2, 64, 0, [255, 66, 94, 255], [0, 0, 0, 0]);
    Render.StringCustom(x + 14, y + 17, 0, name_text, text_color_heimdall, logo_font);
    ...
    renderElements();
    Render.Line(x + 10, y + height - 36, x + width - 10, y + height - 36, line_color);
    var text_size2 = Render.TextSizeCustom("discord.io/heimdall/", menu_font);
    Render.StringCustom(x + width - 15 - text_size2[0], y + height - 26, 0, "discord.io/heimdall/", text_color, text_icon_font);
    Render.StringCustom(x + 15, y + height - 26, 0, "[Update: " + last_upd + "] [Version " + version + " BETA]", text_color, text_icon_font);
    renderHints();
}
...
function renderTabs(tabs) {
    for (i = 0; i < tabs.length; i++) {
        ...
        cursor_pos = Input.GetCursorPosition();
        var font = Render.AddFont("Heimdall Ui", 64, 400);
        var text_size = Render.TextSizeCustom(name, font);
...
function renderSubtabs(subtabs) {
    ...
    for (i = 0; i < subtabs_count; i++) {
        ...
        cursor_pos = Input.GetCursorPosition();
        var x1 = x + 1 + ((subtabs_width / subtabs_count) * i);
        var y1 = subtab_start;
        var x2 = x + 1 + ((subtabs_width / subtabs_count) * (i + 1));
        var y2 = subtab_start + 80;
        if (UI.IsCursorInBox(x1, subtab_start, (subtabs_width / subtabs_count), 80) || subtabs[name][i] == selected_subtabs[name]) {
            if (Input.IsKeyPressed(0x01) && !is_moving) {
        ...
        Render.StringCustom(textX, textY, 12, subtab, color, subtabs_font);
        Render.StringCustom(iconX, iconY, 55, icon, color, icon_font);
        Render.StringCustom(text_iconX, text_iconY, 5, text_icon, color, text_icon_font);
    }
}

function renderBox(name, height, right) {
    box_width = ((width - (15 * 2)) / 2) - 10;
    var boxX = (right) ? x + 15 + box_width + 24 : x + 10;
    var boxY = subtab_start + box_offsets[(right) ? 1 : 0] + 58 + 13;
    ...
    var text_size = Render.TextSizeCustom(name, heimdall_tab);
    var textX = boxX + (box_width / 2) - (text_size[0] / 2);
    
	Render.StringCustom(textX, boxY - (text_size[1] / 2) - 10, 0, name, heimdall_tabcolor, heimdall_tab);
...
function renderCheckbox(name, checkboxY, right, id, hint) {
    ...
    if (UI.IsCursorInBox(checkboxX, checkboxY, box_width - box_margin_x * 2, checkbox_height) && !slider_changing) {
        if (hint !== "") {
            var hint_icon_font = Render.AddFont("Inter", 7, 800);
            Render.StringCustom(elementWidth - 13, checkboxY - 1, 0, "?", text_color, hint_icon_font);
            if (cursor_pos[0] >= elementWidth - 17 && cursor_pos[0] <= elementWidth + 1 && cursor_pos[1] >= checkboxY && cursor_pos[1] <= checkboxY2) {
                ...
                if (hint_x + hint_size[0] + 3 > x + width + 150) {
...
function renderHints() {
    if (hint_text === "") return;
    hint_size = Render.TextSizeCustom(hint_text, menu_font);
    rounded_rect(hint_x, hint_y, hint_size[0] + 16, hint_size[1] + 16, 6, hintbox_color);
    Render.StringCustom(hint_x + 7, hint_y + 8, 0, hint_text, text_color, text_icon_font);
}
